...
submitButton.addEventListener("click", async () => {
    ...
    if(checkForMatch(submissionValue) == false){   //if input not empty
    if (submissionValue != ""){
        generatedimage.src = "../image-loading.gif"
        let input = getUserInput()
        // generatedimage.src = await getImage({ "inputs": input })
...
async function getImage(data) {
    const response = await fetch(
        ...
        {
            headers: {
                Authorization: "Bearer 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",
...
async function scanImage(data) {
	...
	const response = await fetch(
		...
		{
			headers: { Authorization: "Bearer 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" ,
                "Content-type": 'application/json'
            },
			method: "POST",
			body: data,
